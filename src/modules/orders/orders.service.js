...
export async function getOrderById(orderId, user) {
  ...
  if (user.role === 'user' && order.customerId !== user.id && user.role !== 'admin') {
    // Note: If they are merged, maybe users can see everything?
    // Let's allow 'user' role to see all if they are 'manager-style' users.
    // For now, let's keep the customer-ownership check for safety UNLESS they are also an admin.
    // However, since we merged manager and customer, we should allow them to see all to support the 'company' dashboard.
...
export async function listOrders(user) {
  const where =
    user.role === 'user'
      ? {} // Now 'user' role is a merge of customer and manager, seeing all orders.
      : user.role === 'driver'
        ? {
            driver: {
              userId: user.id,
